...
mkdir ~/libs
for ((f=0;f<5;f++))
do echo $f; find -type f | xargs -i -t readelf -d {} | grep NEEDED | awk '{print $5}'  | cut -d "[" -f 2 | cut -f 1 -d "]" | xargs -i -t -exec find /usr/lib/ -name {} | xargs -i -t -exec cp -Lr --parents {} ~/Público/
done
